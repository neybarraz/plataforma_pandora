...
def _render_topo() -> None:
    with st.container(key="solucao_topo_box"):
        render_topo_html(
            "solucao-topo",
            """
            Esta etapa tem como objetivo consolidar, de forma estratégica e objetiva, os principais
            resultados da análise em uma síntese clara e orientada à tomada de decisão. Aqui, o foco
            é transformar dados e interpretações em conclusões técnicas consistentes, destacando os
            elementos mais relevantes, validando hipóteses, estruturando argumentos com base em evidências
            e delimitando as condições de aplicação dos resultados. Trata-se de um momento decisivo, no
            qual o trabalho ganha valor ao apresentar, com precisão e segurança, respostas fundamentadas e aplicáveis.
            """,
        )
...
def _refresh_question_counters(username: str, paginas: list[dict[str, Any]]) -> None:
    st.session_state.solucao_total_questoes = _count_total_questions_from_catalog(paginas)
    st.session_state.solucao_respondidas = _count_answered_questions(username, paginas)
...
def render(
    ...
    if not paginas:
        st.info("Nenhuma página foi configurada para a etapa Solução.")
        return
