...
from apps.app_01.sections.solucao.conteudos.catalago import get_paginas
from apps.app_01.storage import load_user_data, update_question_payload
...
def _inject_visual_css() -> None:
    st.markdown(
        ...
        <style>
            ...
            .solucao-progresso-label {
                margin: 0.25rem 0 0.35rem 0;
                font-weight: 600;
                color: white;
            }

            .st-key-solucao_macro_tabs button[kind="primary"] {
                background-color: #f97316;
                color: white;
                border: 1px solid #f97316;
            }

            .st-key-solucao_macro_tabs button[kind="secondary"] {
                background-color: transparent;
                color: #f97316;
                border: 1px solid #f97316;
            }

            .st-key-solucao_macro_tabs button:hover {
                background-color: #ea580c;
                color: white;
                border: 1px solid #ea580c;
            }

            .st-key-solucao_sidebar_header div[data-testid="stMarkdownContainer"] p,
            .st-key-solucao_content_header div[data-testid="stMarkdownContainer"] p,
            .st-key-solucao_status_box div[data-testid="stMarkdownContainer"] p {
                font-weight: 600;
                margin-bottom: 0.2rem;
            }

            .st-key-solucao_sidebar_buttons button[kind="secondary"] {
                text-align: left;
            ...
            .st-key-solucao_sidebar_buttons button[kind="primary"] {
                text-align: left;
                justify-content: flex-start;
                border: 1px solid #16a34a;
                border-radius: 0;
            ...
            .st-key-solucao_sidebar_buttons button:hover {
...
def _render_topo() -> None:
    st.markdown(
        """
        <div class="solucao-topo">
            Estrutura inicial para a solução técnica do conforto térmico
            em sala de aula, organizada em três eixos: síntese dos resultados,
            decisão técnica e condições de aplicação.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def _status_salvamento() -> None:
    ...
    with st.container(key="solucao_status_box"):
        st.markdown("Status")
...
def _extract_solucao_answers(data: dict[str, Any] | None) -> dict[str, Any]:
...
def _ensure_cache_loaded(
    ...
    if isinstance(saved_data, dict):
        st.session_state.solucao_data_cache = saved_data
    else:
...
def _update_cache_question_payload(question_id: str, payload: dict[str, Any]) -> None:
    ...
    solucao = responses.setdefault("solucao", {})
    if not isinstance(solucao, dict):
        responses["solucao"] = {}
        solucao = responses["solucao"]

    solucao[question_id] = payload
...
def _hydrate_widgets_from_cache(
    ...
    *,
    ...
    data = st.session_state.get("solucao_data_cache", {})
    solucao_answers = _extract_solucao_answers(data)
    ...
    for bloco in conteudo.get("blocos", []):
        ...
        if qid not in solucao_answers:
            continue

        if (not overwrite) and (widget_key in st.session_state):
            continue

        valor = _get_saved_widget_value(solucao_answers[qid])
    ...
    for i, (col, pagina) in enumerate(zip(cols, paginas)):
        with col:
            if st.button(
                ...
                type="primary" if i == st.session_state.solucao_pagina_idx else "secondary",
...
def _render_conteudo_tabs(pagina: dict[str, Any]) -> dict[str, Any]:
    ...
    with st.container(key="solucao_sidebar_header"):
    ...
    with st.container(key="solucao_sidebar_buttons"):
        for i, conteudo in enumerate(conteudos):
            if st.button(
                ...
                key=f"solucao_conteudo_tab_{pagina_id}_{conteudo['id']}",
...
def _questao_respondida(bloco: dict[str, Any]) -> bool:
    ...
    solucao_answers = _extract_solucao_answers(st.session_state.get("solucao_data_cache", {}))
    payload = solucao_answers.get(qid)
...
def _refresh_question_counters(paginas: list[dict[str, Any]]) -> None:
    st.session_state.solucao_total_questoes = _total_questoes(paginas)
    st.session_state.solucao_respondidas = _questoes_respondidas(paginas)


def _render_barra_progresso(paginas: list[dict[str, Any]]) -> None:
    _refresh_question_counters(paginas)

    total = st.session_state.get("solucao_total_questoes", 0)
    respondidas = st.session_state.get("solucao_respondidas", 0)
...
def _save_question(username: str, bloco: dict[str, Any]) -> bool:
    ...
    try:
        ...
        st.session_state.solucao_last_save_label = f"Salvo às {datetime.now().strftime('%H:%M:%S')}"
        st.session_state.solucao_save_error = ""
        return True
...
def _render_questao_texto(
    ...
    with col_save:
        if st.button(
            "Salvar resposta",
            key=f"solucao_save_text_{questao_id}",
            use_container_width=True,
        ):
            _save_text_question_from_button(username, bloco, paginas)

    with col_info:
        saved_data = _extract_solucao_answers(st.session_state.get("solucao_data_cache", {}))
...
def _render_questao_multipla_escolha(
    ...
    with col_save:
        if st.button(
            "Salvar resposta",
            key=f"solucao_save_mcq_{questao_id}",
            use_container_width=True,
        ):
...
def _render_conteudo(
    ...
    with st.container(key="solucao_section_header"):
...
def render(
    ...
    if not paginas:
        st.warning("Nenhuma página de solução cadastrada.")
        return
    ...
    with col_conteudo:
        with st.container(key="solucao_content_header"):
