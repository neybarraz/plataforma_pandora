...
def build_question_id(
    ...
    """
    Padrão oficial:
    <etapa>_<pagina>_<conteudo>_<tipo>_<numero>

    Exemplo:
    investigacao_01_003_q_0001
    memorial_02_001_q_0002
    investigacao_01_003_diag_0001
    """
    etapa = str(etapa).strip().lower()
    tipo = str(tipo).strip().lower()

    if not etapa:
        raise ValueError("etapa é obrigatória para build_question_id.")
    if not tipo:
        raise ValueError("tipo é obrigatório para build_question_id.")

    pagina_str = str(pagina).strip().zfill(2)
    conteudo_str = str(conteudo).strip().zfill(3)
    numero_str = str(numero).strip().zfill(4)

    return f"{etapa}_{pagina_str}_{conteudo_str}_{tipo}_{numero_str}"
...
def normalize_question_payload(
    ...
    *,
    ...
    if tipo == QUESTION_TYPE_MULTIPLA_ESCOLHA:
        ...
        normalized["resposta_escolhida"] = str(
            payload.get("resposta_escolhida", "")
        ).strip()
...
def _is_answered_question_payload(item: Any) -> bool:
    ...
    if tipo == QUESTION_TYPE_MULTIPLA_ESCOLHA:
        return bool(str(item.get("resposta_escolhida", "")).strip())
...
def get_memorial_stats(username: str) -> Dict[str, int]:
    return get_section_stats(username, "memorial")